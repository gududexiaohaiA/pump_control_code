...
class env_SWMM:
    ...
    def reset(self,rainData):#每次使用一次新的降雨数据
        #每一次batch都新生成一个新的降雨
        #每一次reset都赋予新的降雨，新的泵序列
        
#        set_datetime.set_date(self.sdate,self.edate,self.stime,self.etime,self.staf+'.inp')#对self.staf.inp文件修改模拟时间
#        print(self.etime)
...
#        self.rain=change_rain.gen_rain(self.date_t[-1],A,C,P,b,n,R,self.deltt)
        ...
        change_rain.change_rain(rainData,self.orf_rain+'.inp')#将降雨数据先修改self.orf_rain
        change_rain.copy_result(self.staf+'.inp',self.orf_rain+'.inp')#将self.orf_rain.inp复制给staf.inp文件
        change_rain.copy_result(self.orftem+'.inp',self.orf_rain+'.inp')#将self.orf_rain.inp复制给orftem.inp文件
        ...
        self.simulation(self.staf+'.inp')#DDQN模拟修改了rain和时间的inp文件
     
        total_in,flooding,store,outflow,upflow,downflow=get_rpt.get_rpt(self.staf+'.rpt')
        #在确定泵开关之前确定最末时刻（当前）前池水位，水位过低时不开启
        self.pool_d=get_output.depth(self.staf+'.out',self.pool_list,self.date_t[self.iten]-self.iten)#DDQN
        rain_sum=sum(rainData[self.date_t[self.iten]:self.date_t[self.iten+1]])/max(rainData)
        #print(self.date_t[self.iten])
        #print(self.date_t[self.iten+1])
    ...
    def step(self,a,rainData):#对降雨数据进行逐步模拟
        ...
        for pool in self.pool_list:
            #检测水位，根据水位决定泵启停
            flage=0
            if self.pool_d[pool]>(self.limit_level[pool][0]) and self.pool_d[pool]<(self.limit_level[pool][2]):#当前池水位大于最低开泵水位，小于最高水位时
                flage=0
            ...
            if flage==0:
                if a<self.action_space[1]+0.1:#概率小于0.1的时候，不开泵
                    action.append(0)
                    action.append(0)
                elif a>=self.action_space[1]+0.1 and a<self.action_space[1]+0.6:#概率在0.1~0.6的时候，开一个泵
                    action.append(0)
                    action.append(1)
                else:#开两个泵
                    action.append(1)
        ...
        set_pump.set_pump(self.action_seq,self.date_time[1:self.iten],pumps,self.orftem+'.inp')#对self.orftem+'.inp'文件修改泵的策略
        #print(pumps)
        
        tem_etime=self.date_time[self.iten]
        set_datetime.set_date(self.sdate,self.edate,self.stime,tem_etime,self.orftem+'.inp')#在修改泵的策略以后，修改self.orftem+'.inp'文件
        #change_rain.copy_result(infile+'.inp',startfile+'.inp')
        
                
        change_rain.copy_result(self.staf+'.inp',self.orftem+'.inp')#将self.orftem+'.inp'文件复制为self.staf文件
        change_rain.copy_result(self.orftem+'.inp',self.orf_rain+'.inp')#还原 将self.orftem+'.inp'修改为只修改了降雨数据的文件
